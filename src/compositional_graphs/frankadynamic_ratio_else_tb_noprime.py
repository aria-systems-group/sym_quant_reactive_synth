...
class FrankaWorldDynamicRatioTurnBasedElseNoPrime(FrankaWorldDynamicRatioTurnBasedNoPrime):
    """
    ...
    """
    def __init__(self, boxes: int, locs: int, ratio: int, init: tuple, goal: tuple, restricted_human_locs: List[int], restricted_human_boxes: List[int], weight_factor: int = 1, enable_reordering: bool = False):
        super().__init__(boxes, locs, ratio, init, goal, restricted_human_locs, restricted_human_boxes, enable_reordering, weight_factor)
    ...
    def create_sym_weight_dict(self, debug: bool = False, user_random_weights: bool = False):
        ...
        sys_state_evolve_to_human_goal_action = human_goal.vectorCompose(self.latches, list(self.transition_relation.values()))
        ...
        if debug:
            ...
            self.convert_cube_to_state_ADD(self.weight, action=True, verbose=True)
                ...
                for human_box in self.human_boxes:
                    for human_to_loc in self.human_locs:
                        ...
                        if (k == 0 or k % self.ratio != 0) and self.ratio != 0:
                            pred_clause_string = self.xVar_map[f'ready l{self.locs + 1}'] # ready else location
                            for sidx, s in enumerate(pred_clause_string):
                                if s == '1':
                                    self.transition_relation[self.pVars[sidx].bddPattern().__str__()] |= hmove_cube 

                            # update k var - increment k by 1
                            kVal_str = self.kVar_map[f'k{k + 1}']
            ...
            for to_loc in range(1, self.locs + 1):
                ...
                for human_box in self.human_boxes:
                    for human_to_loc in self.human_locs:
                        ...
                        if (k == 0 or k % self.ratio != 0) and self.ratio != 0:
                            pred_clause_string = self.xVar_map[f'holding l{self.locs + 1}'] # holding else location
                            for sidx, s in enumerate(pred_clause_string):
                                if s == '1':
                                    self.transition_relation[self.pVars[sidx].bddPattern().__str__()] |= hmove_cube
    ...
    def test_pre_image(self):
        ...
        preimage = goal_cube.vectorCompose(self.latches, list(self.transition_relation.values()))
