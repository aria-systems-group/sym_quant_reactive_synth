...
class SymbolicPartitionedDFANoPrime():
    """
    Base Class: 
      A class where we constuct a Symbolic DFA in partitioned form, i.e., we ONLY have a set of boolean variables (latches).
        The latches correspond to the current state in the DFA. 
       
      This class is  different from SymbolicDFA in the sense that I create two copies of the current state variables. 
      The PartitionedDFA and ADDPartitionedDFA only create latches and not prime latches. However, I rewrote this class as the input is different.
    ...
    def __init__(self, formula: str, manager: Cudd, latches_map: bidict, game_latches: List[ADD], dfa_name: str = 'dfa', domain: str='manipulator'):
        ...
        # initialize handles for dfa latches and maps
        self.qVars: List[ADD] = []
        # initialize str and sym map
        self.qVar_map = bidict({})
        self.qVar_map_sym = bidict({})
...
class SymbolicPartitionedDFAFromSpotNoPrime(SymbolicPartitionedDFANoPrime):
    """
    This class inherits the SymbolicPartitionedDFANoPrime and implements the following:
...
class SymbolicPartitionedDFAFromMonaNoPrime(SymbolicPartitionedDFANoPrime):
    ...
    def create_dfa_transition_relation(self):
        ...
        """
        self.dfa_transition_relation = {var.bddPattern().__str__(): self.manager.addZero() for var in self.qVars}
        self.dfa_transition_relation_accp_sink = {var.bddPattern().__str__(): self.manager.addZero() for var in self.qVars}  # accepting states transitions are skipped in this TR
        mona_output: str = self.dfa.mona_dfa
