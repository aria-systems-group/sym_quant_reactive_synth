...
This script inherits the Frankaworld() class and augments the class with human moves (`hmove b# l#`). This script implements a Single Manipulator robot (Sys player) operating with **one** human (Env player) in the workspace (*hence dynamic*). The atomic actions for both the players are: 
...
> NOTE 1: FrankaworldDynamic code is a work in progress. This script *tries* to implement a concurrent game (same as our IROS 23; ICRA 22 game) where from a give state, the robot **commits** to an action (say `transit b0`) and the human chooses to either move or not move. Thus, after both the players choose their respective actions, the game evolves to the next Sys player state. This script tries to construct the game compositionally. However, the evolution of the game under transit action for robot and human is incorrect.


#### FrankaworldDynamic_tb - Turn-Based Game

This script implements a Manipulator robot (Sys player) and a human (Env player) operating in a shared workspace as a **pure turn-based (hence tb) game**.  Here Sys player moves first, then the Env player moves. The game start in Sys player state where the Sys **chooses** an action. The game immediately evolves to the Env player state and stays there until (i) human moves a box (`hmove b# l#`) or (ii) robot finishes its execution (either `transit l#` or `transfer l#`). If human moves an object then the game evolves to robot state and the robot `rConf` is updated to its original status (as if the robot is still in its original status).

This abstraction is very permissive in the sense that, after every robot move, we evolve to a human state. If the human chooses to move something, we evolve to a robot state and let it re-evaluate its choices. Thus, this abstraction allow for reactive behaviors as the robot does not *commit* to its action and can chose another action "on-the-fly".

**The problem: With great expressiveness comes great complexity.** The permissive nature of the game makes the existence of a winning strategy for robot impossible. Intuitively, say if the robot commits to action `transit b0`, the human could possibly move either `b0` (or any other box, say, `b1`) to another location and hence ensure that the robot will never reach its intended destination state in the game. In real world, this does not make sense as the robot will eventually finish its action but as the game updates for every human move, the robot could theoretically be stuck in a loop in the game. **To mitigate this we introduce bounded human moves per robot move as mentioned later**. 
...
This script inherits from the base class FrankaWorldDyanmicRatioTurnBased() and overrides the game's update rules as follows: After every human move, the game evolve to robot state where the robot conf. is in else conf. rather than the original conf. For e.g., `(Sys, k0, ready l1, b0 l1) ---- (transit b0) ----> (Env, k0, in-transit b0, b0 l1)` and if human chooses to move then `(Env, k0, in-transit b0, b0 l1) ---- (hmove b0 l2) ----> (Sys, k1, ready else, b0 l2)`. This game semantics captures the robot in motion status more accurately and also makes the game encoding more compact. 

The Abstraction details are similar as above with minor changes:

1.`State Predicate`: For `rConf` predicates that represent valid robot confs. corresponding robot's status of executing an action, we modify them to be as follows. `in-transit b#` to denote robot executing `transit b#` action. Similarly, we introduce `in-transfer to_l#` to denote robot executing `transfer to_l#`. Notice, how we don't keep track of `from` as if the human moves the robot conf. evolves to else state. 


#### Gridworld

This script implement a single agent (Sys player) operating in gridworld. The Sys player can take one step in each cardinal direction with non-deterministic action outcomes modeled as Env player. Specifically, we implement the original game construction (IROS 23) and pure compositional construction. Further, we implement, multiple version of synthesis algorithms - Pure ADD, Hybrid (IROS 23 alg.) with combination of VectorCompose() and Compose() operators. 


# Pure ADD based Strategy Synthesis

1. We implement Pure ADD based strategy synthesis in all script files
2. In FrankaWorld, we experiment with Partitioned as well as Monolithic Preimage Computation and observe that Monolithic is much efficient in preimage computation
3. In FrankaworldDynamic_ratio_else_tb we implement strategy synthesis where the costs are associated with states rather than actions. This is to denote that robot incurs a cost only after it conf. updated from else conf.