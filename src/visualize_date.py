...
LOG_DIR = os.path.join(ROOT_PATH, 'logs/recuv_logs')
...
def plot_synthesis_time(df, target_boxes, target_game, log_plot: bool = False):
    ...
    if log_plot:
        save_plot(file_name=PLOTS_DIR + f'synthesis_time_{target_boxes}b_{target_game}_log.png', plt_handle=plt, fig=plt.gcf())
    else:
        save_plot(file_name=PLOTS_DIR + f'synthesis_time_{target_boxes}b_{target_game}.png', plt_handle=plt, fig=plt.gcf())
